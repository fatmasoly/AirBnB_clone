...
class HBNBCommand(cmd.Cmd):
    ...
    def do_all(self, line):
        ...
        if line is not None and line != "":
            ...
            if args[0] not in storage.classes():
                ...
                new_list = [str(obj)for _, obj in storage.all().items()
                            if type(obj).__name__ == args[0]]
                print(new_list)
        else:
            insta_list = [str(obj)for _, obj in storage.all().items()]
            print(insta_list)
    ...
    def do_update(self, line):
        ...
        Args:
            line (str): The class name, instance id,
            and dictionary representation,
                        separated by spaces.

        Raises:
            KeyError: If the class or instance does not exist in storage.
            ValueError: If the value cannot be
            cast to the attribute's data type.
        """
        parts = line.split()
        if not parts:
            print("** class name missing **")
        ...
        dictionary_str = ' '.join(parts[2:])
        self.fix_dict(class_name, instance_id, dictionary_str)
